...
// Shift Right
// Shifts the bits right, padding the left side with 0's
// Anything that falls off the end is gone
uint32_t SHR(uint32_t x, int n){
  return x >> n;
}

// Rotate right
// Similar to shift but instead the bits that come 
// off the end are put to the start
...
uint32_t sig0(uint32_t x){
  return ROTR(x, 7) ^ ROTR(x, 18) ^ SHR(x, 8);
}
